...
if __name__ == '__main__':
    ...
    print('gat_net.state_dict()')

    import torch.onnx

    # 假设你的模型是gat_net，你需要定义一个适当的输入格式。
    # 这里我们假设模型接受两个输入：特征和邻接矩阵
    input_features = torch.randn(1, 25)  # 假设特征维度是25
    adj_matrix = torch.eye(1)  # 假设邻接矩阵是单位矩阵，表示自循环
    
    # 将模型设为评估模式
    gat_net.eval()
    
    # 导出模型
    torch.onnx.export(gat_net, (input_features, adj_matrix), 'gat_model.onnx',
                  input_names=['input_features', 'adj_matrix'],
                  output_names=['output'],
                  dynamic_axes={'input_features': {0: 'num_nodes'},
                                'adj_matrix': {0: 'num_nodes', 1: 'num_nodes'},
                                'output': {0: 'num_nodes'}},
                  opset_version=13)  # 试试不同的版本如 11, 12, 13 等
